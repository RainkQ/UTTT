...
class SubBoard(object):
    ...
    def add_move_inplace(self, sub_board_coords: SubBoardCoords, player: Player):
        if self.is_finished:
            raise MoveInFinishedBoardError(sub_board_coords, player)
        ...
        self._board[sub_board_coords.row][sub_board_coords.col] = Cell(player)
        self._moves_so_far += 1

        if did_move_win(self._board, sub_board_coords, player):
            self._is_finished = True
            self._winner = player
        elif self._moves_so_far == self._max_moves:
            self._is_finished = True
